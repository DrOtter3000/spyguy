...
@onready var recognition_timer: Timer = $RecognitionTimer

@export var recognition_time := 1.5
...
func _ready() -> void:
	recognition_timer.wait_time = recognition_time
...
func _process(delta: float) -> void:
	if can_see_player():
		if chasing:
			...
			if recognition_timer.is_stopped():
				recognition_timer.start()
			var percentage =(recognition_time - recognition_timer.time_left) / recognition_time
			get_tree().call_group("Player", "update_search_bar", percentage)
	else:
		if chasing:
			if recognition_timer.is_stopped():
				recognition_timer.wait_time = lose_interest_time
				recognition_timer.start()
		else:
			recognition_timer.stop()
			recognition_timer.wait_time = recognition_time
	...
	if recognition_timer.is_stopped():
		get_tree().call_group("Player", "update_search_bar", 0)
...
func _on_recognition_timer_timeout() -> void:
	if chasing:
